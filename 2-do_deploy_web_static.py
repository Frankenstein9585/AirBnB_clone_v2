...
def do_deploy(archive_path):
    ...
    try:
        ...
        run(f'tar -xzf /tmp/{archive_folder}.tgz -C /data/web_static/'
            f'releases/{archive_folder}/')
        run(f'rm /tmp/{archive_folder}.tgz')
        run(f'mv /data/web_static/releases/{archive_folder}/web_static/*'
            f' /data/web_static/releases/{archive_folder}')
        run(f'rm -rf /data/web_static/releases/{archive_folder}/web_static/')
        run('rm -rf /data/web_static/current')
        run(f'ln -s /data/web_static/releases/{archive_folder} '
